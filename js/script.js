...
function setModalWindow(name, number, date, time, target) {
    ...
    modal_number.textContent = number;
    modal_time.textContent = time;
    modal_month.textContent = `${month} ${year}`;
    ...
    btns_name.forEach((name, i) => {
        ...
        btn.addEventListener('click', () => {
            if (name === 'Сохранить') {
                const create_date = {
                    title: modal_input.value,
                    description: modal_textArea.value,
                    phone: modal_inputPhone.value,
                ...
                // fetch(`${URL_BACKEND}/http/event.php`, {
                //     method: 'POST',
                //     body: JSON.stringify(create_date)
                // }); event_id	title	description	phone	color	day	mouth	year	time	
                console.log(date, month, color_div);
