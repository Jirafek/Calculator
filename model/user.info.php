...
class UserInfo {
    ...
    static public function get($id) {
        ...
        $query = $mysqli->query("SELECT `user`.`user_id`, `user`.`login`, `user`.`email`, `user`.`group_id`,`user_info`.* FROM `user` INNER JOIN `user_info` ON `user`.`user_id` = `user_info`.`user_id` WHERE `user`.`user_id` = '$id';");
